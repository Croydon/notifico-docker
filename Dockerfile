...
RUN apt-get update \
    ...
    && wget https://github.com/Croydon/notifico/archive/${NOTIFICO_DOCKER_VERSION}.tar.gz \
    ...
    && python get-pip.py \
    && ln -r -s /notifico/config/config/config.py local_config.py \
    && ln -r -s /notifico/config/config/database.db testing.db \
